...
1. Read `_millhouse/taskmill/backlog.md`. Find the task matching the argument (or if no argument, the first `[p]` task). Extract the task description, any sub-bullets (especially `plan:`), and all context lines.
...
5. Spawn a review agent using the Agent tool with `subagent_type: general-purpose` and `model: sonnet`. Pass the following prompt verbatim, substituting `<TASK_NAME>`, `<TASK_DESCRIPTION>`, `<TASK_SUBBULLETS>`, and `<PLAN_PATH>`:
   ...
   **Then do the following in order:**
   ...
   2. Read the task entry from `_millhouse/taskmill/backlog.md` — task name: `<TASK_NAME>`, description: `<TASK_DESCRIPTION>`, sub-bullets: `<TASK_SUBBULLETS>`
...
7. Run `python ${CLAUDE_SKILL_DIR}/../../scripts/task_subbullet.py _millhouse/taskmill/backlog.md "<task-name>" "review: .llm/reviews/<timestamp>-<slug>.md"` to link the report on the backlog task.
