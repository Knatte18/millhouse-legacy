...
## _millhouse/taskmill/backlog.md (tracked)
...
Completed tasks are deleted from the backlog (via `task_complete.py --delete`) since `_millhouse/taskmill/changelog.md` already records them. The `[x]` state is only used in plan files for step tracking.
...
**Mutation rule:** Never use Edit or Write on `_millhouse/taskmill/backlog.md`. All mutations must go through scripts in `scripts/` (e.g. `task_plan.py`, `task_complete.py`, `task_block.py`). Reading with Read is allowed.

---

## _millhouse/taskmill/changelog.md (tracked)
