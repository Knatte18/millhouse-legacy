...
4. Create `.llm/plans/<timestamp>-<slug>.md` with:
   - **YAML frontmatter:** `started:` (copied from the task's `started:` sub-bullet in `_millhouse/taskmill/backlog.md`) and `finished:` (the timestamp from step 3)
...
7. Run `python ${CLAUDE_SKILL_DIR}/../../scripts/task_plan.py _millhouse/taskmill/backlog.md "<task-name>" <plan-path>` to change state to `[p]` and add/replace the `plan:` sub-bullet. With `--park`: run `python ${CLAUDE_SKILL_DIR}/../../scripts/task_plan.py --state ' ' _millhouse/taskmill/backlog.md "<task-name>" <plan-path>` instead.
...
- Use @taskmill-legacy:legacy-mill-formats skill for plan and backlog format rules.
