...
Python scripts that operate on files with `- [ ]` checkbox format. Same scripts work for `_millhouse/taskmill/backlog.md` and `.llm/plans/*.md`.
...
Shared normalization for `_millhouse/taskmill/backlog.md` formatting. Moved from top-level `backlog_format.py`.
...
With `--delete`: calls `delete_block()` to remove the entry entirely. Used for backlog tasks where `_millhouse/taskmill/changelog.md` already records the completion.
