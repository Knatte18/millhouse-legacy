...
### 0. Hard Invariants (read first, violate never)

These are the rules Thread B historically breaks when it is tired or when a brief clause sounds like an out. Each one exists because of a real incident where Thread B reported a clean completion while silently skipping a step. Read them before anything else.

**I1. Never guess a timestamp.** Every timestamp written to `<STATUS_PATH>` (YAML fields AND the `## Timeline` fenced block) MUST come from a fresh shell call to `date -u`. The two valid invocations are:
- For timeline entries (ISO 8601 with Z): `date -u +"%Y-%m-%dT%H:%M:%SZ"`
- For filename-embedded timestamps (review files, fixer reports): `date -u +"%Y%m%d-%H%M%S"`
...
a. Update `<STATUS_PATH>` YAML code block with current step number and name:
   ...
   ```
   Insert `step-<N>  <timestamp>` in the timeline text block. **Invariant I1:** shell out to `date -u +"%Y-%m-%dT%H:%M:%SZ"` for the timestamp — do NOT guess, infer, or reuse a prior timestamp.
...
**UNKNOWN verdict fallback (C.2).** If verdict is `UNKNOWN`, the reviewer pipeline failed to recover a recognizable verdict from the worker's output. Do not halt — read the review file at `review_file` and parse its YAML frontmatter `verdict:` field (case-insensitive). If the frontmatter reports `APPROVE`, continue to Phase: Finalize. If the frontmatter reports `REQUEST_CHANGES` (or `GAPS_FOUND`), drop into the REQUEST_CHANGES branch below. If the frontmatter `verdict:` field is absent, unparseable, or itself says `UNKNOWN`, halt with `Code reviewer verdict is UNKNOWN and the review file frontmatter is also unparseable. Review file: <path>. Halting; manual intervention required.` Post-W1 this path should be rare — `millpy.core.verdict.extract_verdict_from_text` handles fence-wrapped JSON and YAML frontmatter — but the fallback stays as a defensive belt-and-suspenders.
