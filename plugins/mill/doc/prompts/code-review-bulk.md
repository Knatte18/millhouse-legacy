...
The `<DIFF>` token was removed: a diff is redundant when `<FILES_PAYLOAD>` carries the full file contents, and the diff format adds noise without extra signal.
...
All source files in scope are inlined below. Each file is prefixed with a `=== <relative-path> ===` header and each content line has a 6-character right-justified line number (matching `cat -n` and Claude's Read tool output). Cite findings as `path/to/file.py:42` or `path/to/file.py:42-58` — these citations are load-bearing; the handler verifies each one by re-reading the cited location.
