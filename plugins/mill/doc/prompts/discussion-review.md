...
Dispatched via `millpy.entrypoints.spawn_reviewer --phase discussion`. Synchronous from the caller's perspective.

- **Model:** `review-modules.discussion.default` from config (resolved per round via the rule in `overview.md#config-resolution`). The orchestrator (`mill-start`) passes a reviewer name to `spawn_reviewer`; the entrypoint resolves the model internally.
...
- The reviewer's stdout is a single JSON line: `{"verdict": "APPROVE" | "GAPS_FOUND", "review_file": "<absolute-path>"}`. The discussion-reviewer's verdict vocabulary uses `GAPS_FOUND` instead of `REQUEST_CHANGES` (the verb used by plan and code reviewers).
