...
- **Integration test reachability (BLOCKING):** If any file listed in `## Files` matches `**/tests/integration/**` (or if any step's `Creates:` / `Modifies:` adds a file under a `tests/integration/` directory), the plan frontmatter's `verify:` command MUST exercise that integration suite. Check the `verify:` command text: it must either (a) invoke pytest against the `tests/integration/` directory directly, (b) invoke pytest against a parent directory that covers it (e.g. `pytest tests/` or `pytest .`), or (c) call a wrapper script (flake gate, Makefile target, `uv run pytest <path>`) that transitively runs the integration suite. If `verify:` is scoped narrowly to a single unit-test file while the plan adds an integration test, flag as BLOCKING with the exact finding: "Plan creates integration test `<path>` but `verify:` (`<command>`) does not execute integration tests. Thread B's Phase: Test will never run the integration suite." The suggested fix is to broaden `verify:` to cover both unit and integration scopes. This check exists because Thread B's Phase: Test is scoped to `<VERIFY_CMD>` verbatim — if `verify:` does not include an integration invocation, the integration suite is silently never run.
