...
During Phase: Test, Thread B maintains a `tests_run:` field in the YAML code block listing every test command it actually executed in this phase. Each entry is a shell-quotable command string. Example:

```yaml
tests_run:
  - uv run pytest tests/unit/test_scoring.py
  - uv run pytest tests/integration/test_scoring_vs_baseline.py
```

This field is the transparency signal that `phase: complete` corresponds to "everything the plan asked for was actually executed" rather than "the verify command passed but nothing else was tried." Thread B refuses to write `phase: complete` if `tests_run` does not cover every test file the plan adds under `## Files` (per implementer-brief invariant I2). Skipped commands (e.g. VPN-down) are recorded with an annotation like `(skipped: vpn-unavailable)` — a silent skip is a protocol violation.
