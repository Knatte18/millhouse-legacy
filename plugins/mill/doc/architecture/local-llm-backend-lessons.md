# Lessons learned: local LLM review backend

**Status:** Abandoned. Local LLM as a tool-use review backend is not viable with current-generation small/medium code models. This document captures why, for future reference.

---

## Context

In April 2026 we attempted to add a local LLM backend to Millhouse for plan and code review, with the goal of saving Claude API tokens and speeding up review loops. The first implementation used:
...
| Phase | Model | Mode |
|-------|-------|------|
| Orchestration (mill-start, mill-go session) | Opus | Full tool use |
| Discussion review | Opus (swappable) | Full tool use, independent threads |
| Plan writing | Opus | Full tool use (writes atomic plan) |
| **Plan review** | **Any LLM** | **Bulk single-shot** |
| Implementation | Opus now, cheaper later | Tool use |
| **Code review** | **Any LLM** | **Bulk single-shot** |

Only plan review and code review are candidates for a local or alternative backend. Discussion review stays on Opus because it is open-ended and may need files the author did not pre-list. Orchestration stays on Opus because it is the thinking layer.
...
- **Scope creep during review:** Qwen attempted to create the `plugins/mill/skills/mill-cleanup/` implementation folder during a discussion review (out of scope — discussion review is read-only evaluation).
- **Instruction following:** Qwen ignored "Return only: verdict + file path, no preamble" and dumped the full review inline.

---

## Cost / benefit of continuing

Not worth pursuing further with current small/medium code models via tool-use dispatch:

- **Cost incurred to prove it doesn't work:** WSL2 + CUDA toolkit installation (~2 hours), vLLM install + model download (~20 GB), start/stop scripts, forwarding wrappers, config schema, dispatch protocol, SKILL.md integration points in mill-start / mill-go / mill-setup — all of it for ~30 minutes of actual benchmark data that showed the approach doesn't work.
- **Cost avoided by stopping now:** further SKILL.md complexity, protocol-per-backend adapters, debugging tool-use quirks for each new provider.
- **What we keep:** this document, and the tiered-model architecture and bulk-mode design described above. When the time comes to add an alternative backend, it should be a bulk-mode dispatcher from day one, not a tool-use adapter.

---

## Removed artifacts

The following files were removed after this retrospective was written (April 2026):

- `plugins/mill/scripts/spawn-agent.ps1`
- `plugins/mill/scripts/start-qwen.ps1`
- `plugins/mill/scripts/stop-qwen.ps1`
- `plugins/mill/doc/guides/benchmarks.md`
- `plugins/mill/doc/guides/claude-code-local-llm.md`
- `plugins/mill/doc/guides/getting-started-local-llm.md`
- `plugins/mill/doc/guides/ollama-guide.md`
- `plugins/mill/doc/guides/vllm-guide.md`
...
The benchmark primary sources (`qwen-report.md`, `logging.md`, `qwen_vllm_log.txt`) at the repo root were also removed; their essential content is summarized in this document.
