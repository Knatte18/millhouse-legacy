...
Check that `_millhouse/backlog.kanban.md` exists. If it does not exist, stop and tell the user to run `mill-setup` first.

Read `_millhouse/backlog.kanban.md`. For each `##` column (Backlog, Spawn, Delete), count `###` headings (tasks).

Read `_millhouse/scratch/board.kanban.md` if it exists. For each `##` column (Discussing, Planned, Implementing, Testing, Reviewing, Blocked), count `###` headings (tasks). If `board.kanban.md` does not exist, show all work columns as 0 tasks.
...
2. Try to read `<worktree-path>/_millhouse/config.yaml` for `git.parent-branch`. If not found, fall back to `parent:` in `<worktree-path>/_millhouse/scratch/status.md`. If neither exists, fall back to `main`.
3. Try to read `<worktree-path>/_millhouse/scratch/status.md` for that worktree's phase and step progress.
...
```
Backlog (_millhouse/backlog.kanban.md):
  Backlog:       N tasks
  Spawn:         N tasks
  Delete:        N tasks

Work (_millhouse/scratch/board.kanban.md):
  Discussing:    N tasks
  Planned:       N tasks
