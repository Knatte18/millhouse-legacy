...
Post-write validation for the kanban board files and `_helm/config.yaml`. Skills that write to these files must validate after writing. Rules are structural only — they catch broken file format, not invalid metadata values.

## Backlog Board (`kanbans/backlog.kanban.md`)

### File validation

1. **File must exist.** `kanbans/backlog.kanban.md` must exist. It is git-tracked. On fresh clones, the file exists from git checkout. If missing, run `helm-setup`.
...
## Work Board (`kanbans/board.kanban.md`)

### File validation

1. **File must exist** (when validating after a write). `kanbans/board.kanban.md` is gitignored and local-only. On fresh clones, the file will not exist until `helm-setup` is run or `helm-start` creates it on first task claim.
...
3. **GitHub section (if present).** If a `github:` key exists, it must contain `owner` and `repo` sub-keys. Other sub-keys under `github:` are silently ignored (obsolete fields from prior versions are not an error).
